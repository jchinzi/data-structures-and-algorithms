...
const oddValues = (arr) => {
  let newArr = arr.filter(number => number % 2 === 1);
  return newArr;
};
...
const getCharactersWithoutChildren = (arr) => {
  let childlessArr = arr.filter(person => !person.children);
  return childlessArr;
};
...
const evenOddNumericValues = (arr) => {
  let allNumArr = arr.filter(each => typeof each === 'number');
  let wordVersion = allNumArr.map(num => {
    if (num % 2 === 0){
      return 'even';
    } else return 'odd';
  })
