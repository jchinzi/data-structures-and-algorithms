...
const createServer = () => {
  ...
  app.delete('/things/1', (request, response) => response.status(405).send('Delete It!'));
...
const toTitleCase = (arr) => {
  ...
  arr.forEach(word => {
    ...
    capWords.push(firstLetter + restOfWord);
  })
  return capWords
};
...
let biggerThanLuke = (arr) => {
  ...
  arr.forEach(character => {
    if (parseInt(character.mass) > lukeMass){
      biggerArr.push(character.name);
    }
  })
  let results = biggerArr.join(' - ');
  return results;
};
