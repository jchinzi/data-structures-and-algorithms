...
const charCode = (arr) => {
  let charCodedArr = arr.map(each => each.charCodeAt(0));
  return charCodedArr;
};
...
const extractAbilities = (arr) => {
  let justAbilites = arr.map(each => each.ability.name);
  return justAbilites;
};
...
const extractStats = (arr) => {
  let comboArr = arr.map(each => {
    let comboStat = (each.effort + each.baseStat);
    return {name: each.stat.name, total: comboStat};
  })
  return comboArr;
};
